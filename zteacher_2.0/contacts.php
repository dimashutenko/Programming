<!DOCTYPE html>
<html lang="ua">
  <head>
    <meta http-equiv="Content-Type" content="text/html; charset=utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
    <?php include ('head.php'); ?>
    <link rel="shortcut icon" href="img/the.ico" type="image/x-icon">
    <meta name="description" content="Zteacher Language School"> <!-- Whats app description-->
    <meta name="keywords" content="zteacher, English, learning">
    <meta property="og:title" content="Zteacher Language School Contacts">
    <meta property="og:description" content=""> <!-- bottom-->
    <meta property="og:type" content="article">
    <meta property="og:image" content="img/the.png">
    <meta property="og:site_name" content="Zteacher Language School">
    <title>Zteacher Language School Contacts</title>
    <body>

      <?php include ('trial_lesson_modal.php'); // запись на пробный modal ?>

      <?php include ('navigation.php'); ?>
      
      <section class="section" id="contacts" data-aos="fade-in">
        <div class="container">
          <?php include ('socials.php'); ?>
          <a href="https://www.instagram.com/zteacher_language_school/" class="instagram-link">
            <p class="">Наші інтерактивності в Instagram</p>
          </a>
          <button class="btn-general" type="button" data-bs-toggle="modal" data-bs-target="#trial-lesson-modal">
            Дізнатися більше
          </button>
        </div>
      </section>

      <?php include ('footer.php'); ?>
      
    </body>
  </head>
</html>
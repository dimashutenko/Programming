<section class="section" id="approach">
  <div class="container">
    <div class="row h-100">
      <div class="col-md-6">
        <div class="approach__left h-100 d-none d-md-block" data-aos="fade-right"></div>
      </div>
      <div class="col-12 col-md-6">
        <div class="approach__right approach__right-all d-flex" data-aos="fade-in">
          <h2 class="section-headline d-none d-md-block blue">Наш підхід - це цікавий контент</h2>
          <!-- <h2 class="section-headline headline_700 d-block d-md-none">Підхід</h2> -->
          <div class="school-advantage school-advantage-speaking" data-aos="zoom-in-up">
            <div class="circle"><span class="percentage">90%</span></div>
            <h3 class="school-advantage__title">Говоріння</h3>
            <script>
              approach__more__text = (target, id) =>{ 
                target.classList.add("d-none");
                document.getElementsByClassName(id)[0].getElementsByClassName('school-advantage-mobile-description')[0].classList.remove("d-none");
                  
              }
            </script>
            <a href="javascript:;" class="approach__more__text d-flex d-sm-none" onclick="approach__more__text(this, 'school-advantage-speaking')">Детальніше</a>
            <div class="school-advantage-mobile-description d-none">
              <p class="approach__text">
                Ми хочемо, щоб кожна секунда уроку була вичерпно корисно проведена.
              </p>
              <p class="approach__text">
                Тож урок призначений для говоріння, відпрацювання, запам’ятовування.
              </p>
            </div>
          </div>
          <div class="school-advantage school-advantage-writing" data-aos="zoom-in-up">
            <div class="circle"><span class="percentage">10%</span></div>
            <h3 class="school-advantage__title">Писання</h3>
            <a href="javascript:;" class="approach__more__text d-flex d-sm-none" onclick="approach__more__text(this, 'school-advantage-writing')">Детальніше</a>
            <div class="school-advantage-mobile-description d-none">
              <p class="approach__text">
                Ми розуміємо, що писати — це важливе уміння, особливо в теперішній час. Більшість спілкування між друзями, колегами, партнерами, клієнтами — це texting.
              </p>
              <p class="approach__text">
                Тому — creative writing, essays, messages, stories — все це включено в навчальний процес. Але пишемо ми менше 10% уроку, решта письмових завдань — це цікаві, креативні домашні завдання з обов'язковим feedback від учителя.
              </p>
            </div>
          </div>
          <div class="school-advantage school-advantage-interest" data-aos="zoom-in-up">
            <div class="circle"><span class="percentage">100%</span></div>
            <h3 class="school-advantage__title">Фокус на інтереси студента</h3>
            <a href="javascript:;" class="approach__more__text d-flex d-sm-none" onclick="approach__more__text(this, 'school-advantage-interest')">Детальніше</a>
            <div class="school-advantage-mobile-description d-none">
              <p class="approach__text">
                Кожна людина, яка приймає рішення вчитися, незалежно від віку, заслуговує безмежної поваги.
              </p>
              <p class="approach__text">
                Кожен має право нічого не знати, або знати трішки, робити безліч помилок, боятися, соромитися... Ми все розуміємо і працюємо для вас!
              </p>
            </div>
          </div>
        </div>
        <div class="approach__right approach__speaking d-none">
          <div class="approach__right__close"></div>
          <h2 class="approach__title">
            Ви будете розмовляти!
          </h2>
          <div class="school-advantage">
            <div class="circle"><span class="percentage">90%</span></div>
            <h3 class="school-advantage__title">Говоріння</h3>
          </div>
          <h3 class="approach__text">Ми хочемо, щоб кожна секунда уроку була вичерпно корисно проведена.</h3>
          <h3 class="approach__text">Тож урок призначений для говоріння, відпрацювання, запам’ятовування.</h3>
        </div>
        <div class="approach__right approach__writing d-none justify-content-center">
          <div class="approach__right__close"></div>
          <h2 class="approach__title">
            Ви, звісно, будете писати!
          </h2>
          <div class="school-advantage">
            <div class="circle"><span class="percentage">10%</span></div>
            <h3 class="school-advantage__title">Писання</h3>
          </div>
          <h3 class="approach__text">
            Ми розуміємо, що писати — це важливе уміння, особливо в теперішній час. Більшість спілкування між друзями, колегами, партнерами, клієнтами — це texting.
          </h3>
          <h3 class="approach__text">
            Тому — creative writing, essays, messages, stories — все це включено в навчальний процес. Але пишемо ми менше 10% уроку, решта письмових завдань — це цікаві, креативні домашні завдання з обов'язковим feedback від учителя.
          </h3>
        </div>
        <div class="approach__right approach__interest d-none justify-content-center">
          <div class="approach__right__close"></div>
          <h2 class="approach__title d-flex text-center">
            Цікавий матеріал, що підсилє бажання вчитися!
          </h2>
          <div class="school-advantage" style="margin:2rem 0;">
            <div class="circle"><span class="percentage">100%</span></div>
            <h3 class="school-advantage__title">Фокус на інтереси студента</h3>
          </div>
          <h3 class="approach__text">
            Любов до учнів! 
            Повага до учнів! Прийняття!
          </h3>
          <h3 class="approach__text">
            Кожна людина, яка приймає рішення вчитися, незалежно від віку, заслуговує безмежної поваги. 
          </h3>
          <h3 class="approach__text">
            Кожен має право нічого не знати, або знати трішки, робити безліч помилок, боятися, соромитися... Ми все розуміємо і працюємо для вас!
          </h3>
        </div>
      </div>
    </div>
  </div>
</section>
...
function copy_fonts_museo(cb){
	gulp.src('./fonts/MuseoSansCyrl/*')
   	.pipe(gulp.dest('./dist/fonts/MuseoSansCyrl'));
   	cb();
}
...
function copy_img(cb){
	gulp.src('./img/*.{png,jpg,gif,jpeg,mp4,webp}')
   	.pipe(gulp.dest('./dist/img/'));
   	cb();
}

//changes path and multiple styles, scripts to one
function ref(cb){
    gulp.src('./*.html')
        .pipe(useref())
...
exports.build = gulp.series(clean, ref, copy_img, copy_fonts, copy_fonts_museo) //command: gulp build
